...
out_dir = projectdir() * "/data/image_binary/"
...
jldopen(file, "r") do fin
    ...
    # output files
    out_file = joinpath(out_dir, "image_pairs_" * vars * ".jld2")
        ...
        for p in 1:n_pairs
            ...
            if save_pngs == true
                save_image_png(imgA, imgB, p;
                    out_dir = joinpath(projectdir(), "data", "visual", vars),
                    name = "pair")
            elseif p == 1 || p == n_pairs
                save_image_png(imgA, imgB, p;
                    out_dir = joinpath(projectdir(), "data", "visual", vars, "preview"),
                    name = "pair")
            end
        end
    end
